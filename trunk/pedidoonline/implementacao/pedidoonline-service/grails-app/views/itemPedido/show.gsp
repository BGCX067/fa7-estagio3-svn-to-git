	...
	<body>
		...
		<div id="show-itemPedido" class="content scaffold-show" role="main">
			...
			<ol class="property-list itemPedido">
				...
				<g:if test="${itemPedidoInstance?.item}">
				<li class="fieldcontain">
					<span id="item-label" class="property-label"><g:message code="itemPedido.item.label" default="Item" /></span>
					
						<span class="property-value" aria-labelledby="item-label"><g:link controller="item" action="show" id="${itemPedidoInstance?.item?.id}">${itemPedidoInstance?.item?.encodeAsHTML()}</g:link></span>
